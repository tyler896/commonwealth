...
export function ProductCard({
  ...
  return (
    <article
      className={`group flex h-full flex-col border border-line bg-paper transition hover:border-brand-blue/40 ${
    ...
    >
      <Link to={`/shop/${product.id}`} className="block min-w-0">
        ...
        <h3
          className={`font-blackletter text-ink transition group-hover:text-brand-blue-deep ${
            compact
      ...
      <div className="mt-auto flex items-center justify-between gap-2 pt-4 md:gap-3 md:pt-5">
        ...
        <span className="shrink-0 rounded-full border border-brand-red/30 bg-brand-red/10 px-2.5 py-1.5 font-display text-[9px] tracking-[0.14em] uppercase text-brand-red-deep md:px-4 md:py-2 md:text-[10px] md:tracking-[0.18em]">
          Coming Soon
        </span>
      </div>
    </article>
